...
class RDP_WIKI_EMBED_CONTENT {
    ...
    private function preRender() {
        ...
        if(empty($this->_options['title_show'])) array_push($remove_elements, '#firstHeading');         
        if(empty($this->_options['toc_show'])) array_push($remove_elements, '#toc'); 
        if(empty($this->_options['infobox_show'])) array_push($remove_elements, '.infobox', '.infobox_v2', '.infobox_v3'); 
        ...
        if(isset($this->_options['hide_html'])){
            $html_elements = preg_split( '/\r\n|\r|\n/', $this->_options['hide_html'] );
            foreach ($html_elements as $element) {
                array_push($remove_elements, $element);
            }             
        }
    ...
    private function scrub(&$body) {
        ...
        foreach($body->find('a') as $link){
            ...
            if(is_array($oQueryPieces) && isset($oQueryPieces['redlink']) && strtolower($oQueryPieces['redlink']) == '1'){
                $link->href = $this->buildLink($oLinkPieces, $oURLPieces);
                continue;
            }
    ...
    private function buildLink($oLinkPieces,$oURLPieces) {
            if(key_exists('path', $oLinkPieces)):
                $sPath = $oLinkPieces['path'];
                if(substr($sPath, 0, 3) == '../')$sPath = substr($sPath, 3);
                if(substr($sPath, 0, 2) == '..')$sPath = substr($sPath, 2);
            ...
            return RDP_WIKI_EMBED_UTILITIES::unparse_url($oLinkPieces);
    }//buildLink
