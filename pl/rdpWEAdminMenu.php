...
class RDP_WIKI_EMBED_ADMIN_MENU {
    ...
    static function admin_page_init(){
        ...
        add_settings_field(
            'hide_html',
            esc_html__('Hide HTML elements','rdp-wiki-embed'),
            array('RDP_WIKI_EMBED_ADMIN_MENU', 'hide_html_input'),
            'rdp-wiki-embed',
            'rdp_we_defaults'
        );        
        // links
    ...
    static function toc_show_input() {
        ...
        $value = intval($value);        
        echo '<lable>';
        echo '<input type="checkbox" value="1" name="rdp_wiki_embed_options[title_show]" ' . checked( $value , 1, false) . '/> ';
        esc_html_e('Display title','rdp-wiki-embed');
        echo '</lable>';
        echo '<br /><br />';
    ...
    static function hide_html_input() {
        $default_settings = RDP_WIKI_EMBED_PLUGIN::default_settings();
        $sLabel = esc_attr__('Separate CSS selectors by new lines', 'rdp-wiki-embed');
        $options = get_option( 'rdp_wiki_embed_options' );
        $text_string = isset($options['hide_html'])? $options['hide_html'] : $default_settings['hide_html'];
        $text_string = esc_textarea($text_string);
        echo '<span class="alignleft">' . $sLabel . '</span><br />';
        echo '<textarea name="rdp_wiki_embed_options[hide_html]"  rows="5" cols="50">' . $text_string . '</textarea>';        
    }//hide_html_input
    ...
    static function options_validate($input){
 	...
 	$options = array(
                ...
                'hide_html' => (isset($input['hide_html']) ? $input['hide_html'] : $default_settings['hide_html']),
